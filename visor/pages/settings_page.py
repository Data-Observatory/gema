...
Providers and their keys are one single block, not two separate ones:
every already-configured provider renders as an editable row (Base URL +
API key — its name is its identity, not editable inline, since agents
reference providers by name and renaming here could silently dangle that
reference) directly inside the "Providers" card, right above the
"Add a provider" mini-form that appends a new row (picked from the
config/providers.yaml pool, which autofills its connection details, or
fully custom). Nothing is written until Save & Continue, same as every
        ...
        def body() -> None:
            ...
            with ui.card().classes("w-full q-mt-md"):
                ...
                for provider in pipeline_config.providers:
                    ...
                    with ui.row().classes("w-full items-end q-mt-sm"):
                        ui.label(provider.name).classes("w-32 text-bold")
                        url_input = (
                    ...
                    ui.label(hint).classes("text-caption q-mb-sm")
                    url_inputs[provider.name] = url_input
                    env_inputs[provider.api_key_env] = key_input

                ui.separator().classes("q-my-md")

                ui.label("Add a provider").classes("text-subtitle2 text-bold")
                ui.label(
