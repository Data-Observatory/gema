...
# dist/GemaVisor.app. Uses hdiutil (built into every macOS runner/machine) —
# no extra dependency. See visor/BUILD.md.
set -euo pipefail

APP_PATH="dist/GemaVisor.app"
DMG_PATH="dist/GemaVisor.dmg"
...
rm -f "$DMG_PATH"
hdiutil create -volname "Gema Visor" -srcfolder "$APP_PATH" -ov -format UDZO "$DMG_PATH"
echo "Built $DMG_PATH"
