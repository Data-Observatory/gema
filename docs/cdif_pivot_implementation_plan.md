...
Read-only per the plan (B4 is "deferred/needs-data" — no fix attempted). `schema:distribution` and `schema:variableMeasured` are both owned by `media_files` (confirmed against `config/agents.yaml`'s `fields:` list for that agent — matches this doc's own ownership sentence above, "`creators_publishers` owns creator/publisher/contributor, `rights_funding_citations` …"; CLAUDE.md has no per-field ownership table).
...
Report generated locally at `reports/live_eval_20260907_093322.md`, **not committed** (`reports/` is gitignored) — the per-fixture table above is the durable record; the file itself won't survive a fresh clone and every `reports/*.md` path cited in this doc (including the two all-zero reports referenced above) should be read the same way.

## JS-render fallback via `obscura` (2026-09-09)

Follow-on from the Post-PR#45 investigation above: Phase A (content-fetch → markdown) concluded "do not proceed to A2" because format alone (markdownify/trafilatura/html2text) didn't beat gema's own extractor on real traffic. Re-opened from a different angle: 3 of that bake-off's 6 golden URLs (`sample01`, `sample02`, `sample06`) returned near-nothing from *every* static-HTML method because they're JS-rendered SPAs with no real content in the static HTML at all — not a parsing-quality gap, a rendering gap.

**Evaluated and rejected:** vendoring/porting CODATA's own `semantic-croissant` scraper (owner has usage clearance for that repo specifically, so this was judged purely on merits, not license) — its HTML→text step is just `markdownify` (already bake-off'd, no win), and its bot-challenge/JS-render layer is `cloudscraper` + `Playwright`. `cloudscraper` (bot-challenge bypass) stays out — gema fetches public data-portal landing pages, not scraping bot-walled sites at scale, so there's no matching need and it's a real ToS/ethics surface. `Playwright` itself was also rejected: heavy new dep + full Chromium binary install, breaks the "no new runtime deps"/CI-simplicity posture.

**Chosen instead: `obscura`** (https://github.com/h4ckf0r0day/obscura, Apache-2.0, standalone Rust CLI, ~30MB RAM / ~70-80MB binary, no Python package, no browser install) — a real JS engine (V8) at a fraction of Playwright's weight. Verified empirically, live, against real golden-fixture URLs (not estimated): `obscura fetch --dump html` piped through gema's own `clean_html_to_text` —
- `rasgos.cl` (sample02): 9 chars (current static fetcher) → 269 chars of real content, via a real `<main>`/`<article>` tag gema's own extractor picked up cleanly (no nav/chrome bleed).
- `datos.gob.cl` (sample01): 72 chars → 425 chars of real structured content (author, license, update frequency, ...), same clean extraction.

One extraction implementation, not two: obscura's only job is running JS; gema's existing `clean_html_to_text` still owns "what counts as content" (validated directly rather than trusting obscura's own `--dump text` mode, which was the first thing measured but never what got shipped).

**Adversarial review (Opus) before any code was written** surfaced two real blockers, both resolved by direct owner decision, not worked around silently:
1. *Deployment of the binary into Visor* — the plan originally assumed "reuse Visor's existing first-run seeding," which turned out to be false (`bootstrap.py`'s seeding just copies a bundled text file, no network/download machinery at all) — and a runtime-downloaded, unsigned/non-notarized binary hits Gatekeeper quarantine on macOS. **Resolved: bundle at build time instead**, via `visor.spec`'s existing (previously empty) `binaries` hook — avoids the runtime-download problem entirely, since the binary ships inside the same (already unsigned) `.app`/installer as everything else.
2. *Unverified pipeline* — the measured numbers above (356/803 chars) were from `--dump text`; the proposed design was `--dump html` → `clean_html_to_text`, never measured. **Resolved by testing it directly** (269/425 chars, see above) before committing to the design.

Also resolved directly, no code-behavior ambiguity left open: a `None` return from the static fetch (404, non-HTML content-type) is **not** retried via render — a dead resource or a PDF doesn't become a webpage by rendering it; only a timeout/connection-error or a too-thin-but-real static result triggers the fallback. No pinned-checksum concern for supply-chain risk: obscura's GitHub release API exposes a real per-asset sha256 digest (no separate `SHA256SUMS` file, but the same data), so the exact tag (`v0.2.2`) and sha256 per platform are hardcoded in `fetch_obscura.py`, verified before extraction, never after.

**Implemented** (branch `feat/content-fetch-js-render-fallback`, off `dev`):
- `content_fetcher.py`: `_MIN_STATIC_TEXT_LEN` (200 chars, matching the real dead/live-page gap observed) gates the fallback; `_fetch_via_obscura()` subprocess-wraps `obscura fetch --dump html`, fail-soft (missing binary, non-zero exit, timeout all degrade to `None` + a warning, same contract as the rest of this module); `fetch_page_content()` gained `js_render_fallback: bool = False`.
- `PipelineConfig.enable_js_render_fallback: bool = False` — config-file-only, no dedicated CLI flag, mirrors `enable_content_fetch`'s existing pattern exactly (only consulted when that flag is also on).
- `pipeline.py`'s `_maybe_fetch_content` passes the flag through; `content_fetcher.py` itself stays config-free (accepts it as a parameter, doesn't import `config/`).
- `visor/installer/fetch_obscura.py` — the build-time vendoring step: downloads the pinned, sha256-verified, render-enabled (non-stealth, non-`-no-render`) asset for the running platform, extracts, sets the exec bit, atomic-renames into place; idempotent (skips the network call if already staged). Raises `ObscuraUnsupportedPlatformError` for a platform with no pinned asset — `visor.spec` treats that as "skip bundling for this build," never a hard failure, since the flag is opt-in and off by default.
- `visor.spec`'s `binaries` list now includes the staged obscura binary at the bundle root (where `content_fetcher.py`'s `_obscura_binary_path()` looks for it under `sys._MEIPASS`) — no CI workflow changes needed, since PyInstaller executes the whole spec file as a script, so the fetch+stage step runs automatically as part of the existing `uv run pyinstaller visor/visor.spec` step in both `visor-build.yml` and `release.yml`.
- Visor UI: new "Render with JS when the page comes back empty" checkbox in the Agents page's pipeline-behavior card, disabled whenever content-fetch itself is off (it's a sub-option). Wired through all 5 real touchpoints (checkbox render, save handler, upload apply-back, settings persistence dict, `PIPELINE_BEHAVIOR_FLAGS` tuple in `visor/settings.py`) — the upload apply-back and settings-persistence dict are both hand-written per-field lists with no test enforcing completeness, exactly the kind of spot a new flag has silently gone missing before (`validate_shacl_conformance`'s own history in this doc); covered by dedicated regression tests for both this time.
- Full test coverage: `tests/test_content_fetcher.py` (binary-missing/non-zero-exit/timeout/empty-output/threshold-trigger/`None`-vs-thin-string distinction, all via mocked `subprocess.run`), `tests/test_pipeline_integration.py` (flag pass-through), `visor/tests/test_fetch_obscura.py` (real tar.gz/zip extraction + sha256 mismatch, mocked network), `visor/tests/test_agents_page.py` + `test_ui_navigation.py` (upload round-trip, save/download round-trip, enabled/disabled UI binding).
- Golden fixtures: **not re-recorded** — flag defaults `False`, so `fetch_page_content`'s cache-key-relevant behavior (and every cached LLM response keyed off the resulting prompt) is unaffected; confirmed via `cache.py`/`factory.py`'s key composition, neither reads `PipelineConfig`. Flipping the flag on for the golden set specifically is separate, deferred future work — re-baking `fetched_content` for `sample01`/`sample02` under it would force a full `make record-golden` + fresh live-eval baseline, not a mechanical fixture edit.
- Verification: `ruff check src/ tests/ scripts/ visor/`, `mypy src/ scripts/`, `mypy visor --exclude visor/tests` (all clean); `pytest -m "not live"` (1390 passed); `pytest visor/tests -p nicegui.testing.user_plugin -o asyncio_mode=auto -m "not live"` (239 passed); a full local `make build-visor` (real network, real `uv sync --extra visor --group visor-build`) — confirmed the whole chain end-to-end: `ensure_obscura` staged the real binary (fresh download, real sha256 match), PyInstaller's onedir COLLECT placed it at `GemaVisor/_internal/obscura` (that's where onedir mode's `sys._MEIPASS` resolves at runtime, not the top-level dir), and it executes correctly from there (`obscura 0.2.2`). Build output (`build/`, `dist/`, `vendor/`) cleaned up afterward, none of it committed.

**Second adversarial review (Opus) against the actual diff**, after the code above was written — verified all counts/checks itself rather than trusting the claims, then found several real gaps, all fixed:
- The two new files (`fetch_obscura.py`, `test_fetch_obscura.py`) were never `git add`ed — a real near-miss, since a commit as-is would have shipped a `visor.spec` importing a module absent from the repo.
- `visor.spec` conflated two different failure modes under one exception type: a sha256 mismatch (a real tamper/corruption signal) was silently swallowed the same way as "no pinned asset for this platform" (a legitimate skip). Split into `ObscuraUnsupportedPlatformError` (platform-unsupported *or* a transient network failure fetching the release — both genuinely skip-worthy) vs. a new `ObscuraVerificationError` (sha256 mismatch, or an archive missing its expected binary member) that `visor.spec` deliberately does *not* catch, so it fails the build loudly instead.
- `_fetch_via_obscura` passed any string through to obscura's argv verbatim — closed with an explicit `http(s)://` scheme guard (defense in depth; the trigger-logic fix below already prevents non-http(s) strings from reaching it via the only real call site, since httpx itself rejects them before `render_eligible` is ever set).
- The render trigger fired on *any* fetch exception, including a flat connection error/DNS failure — wasteful, since obscura hits the identical network path and would just burn a subprocess timeout confirming the same dead host. Narrowed to `httpx.TimeoutException` only (a slow-but-reachable page); a connection error/other request failure is now trusted as-is, same as a 404.
- `ensure_obscura` never re-verified an already-staged binary against `OBSCURA_VERSION` — a version bump wouldn't force a re-fetch on a machine (or warm CI cache) that already had an old one staged. Fixed by keying the vendor path on the version (`vendor/obscura/<version>/obscura`).
- `str(int(timeout))` for obscura's own `--timeout` flag produced a nonsensical `--timeout 0` for any sub-1-second timeout — floored to `max(1, ...)`.
- Test gaps closed: the frozen-build branch of `_obscura_binary_path()` (both staged and skipped-staging cases) had zero coverage despite being the entire point of the Visor bundling effort; `_fetch_via_obscura`'s actual argv (including the resolved URL and `--timeout` value) was asserted loosely; the exact-threshold boundary, the bare-DOI-resolved-before-render case, and the "render result shorter than static, keep static" branch were all exercised by hand during review but not pinned by a test.
- `enrichers/AGENTS.md` — the detailed per-directory guidance layer per `CLAUDE.md` — hadn't been updated in its three `enable_content_fetch` mentions; added `enable_js_render_fallback` alongside each. `.gitignore`'s `vendor/` pattern anchored to `/vendor/` (repo-root only, not unintentionally matching a same-named directory elsewhere).
- Confirmed correct on inspection and by direct testing, no change needed: archive extraction never calls `extractall()`/trusts an archive-supplied path (no Zip Slip/traversal risk), the temp-dir-then-atomic-rename staging never leaves a partial file at the final path on any failure, and all 5 Visor UI touchpoints were already wired consistently.
- Re-verified after all fixes: `ruff`/`mypy` clean, `pytest -m "not live"` and the visor suite both pass (counts grew with the new tests — see the test files themselves for current totals, not restated here since they'll drift).
