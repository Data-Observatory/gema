...
## Step 3 — DataCite exporter — DONE

- [x] `exporters/datacite.py`: real CDIF→DataCite mapping table (Q2 table read in reverse — `_build_resource`/`_build_titles`/`_build_creators`/`_build_publishers`/etc.), `DataCiteExportResult{datacite_json, warnings, token_usage}`, delegates to `DataCiteSchema46` normalizers/`validate_output` only as the last step (never copy-pasted normalizer logic)
