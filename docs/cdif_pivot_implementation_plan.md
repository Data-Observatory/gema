...
**Judgment call a reviewer should double check**: the vendored `schema.json`'s own `schema:creator` field description asks for the JSON-LD `{"@list": [...]}` construct (to preserve author order), matching the original Q2 table's C4 note — but `CDIFDiscoveryProfile` (Step 2, already landed) actually emits `schema:creator` as a **bare list** in real generated output (confirmed against `tests/fixtures/golden/expected/*.json`). This is a real, still-open gap between the vendored spec and this repo's Step 2 generation code, not something to silently paper over. `exporters/datacite.py`'s `_creator_list()` helper accepts *either* shape defensively (bare list today, `{"@list": [...]}` if Step 2's generation is ever fixed to match the spec) rather than picking one and dropping every creator on a document shaped the other way. Not resolved as part of this step — tracked here so it isn't lost; the actual fix (if wanted) belongs in `CDIFDiscoveryProfile`'s `schema_creator` field/normalizer, not in this exporter.

## Step 4 — Croissant exporter

- [x] Resolve Open Question #9 (Croissant top-level field mapping — direct spec check): verified against `docs/croissant-spec-1.1.md` in `mlcommons/croissant`, commit `401f6fff81db26a49c0d1704f02bffc4e4fa8fe2`, fetched 2026-09-04. Verified field list (corrected from the earlier draft — see the Q9 paragraph above for what changed and why): **Required** — `@context, @type ("sc:Dataset"), dct:conformsTo (aliased "conformsTo"), name, description, license, url, creator, datePublished, distribution`. **Recommended** — `keywords, publisher, version, dateCreated, dateModified, sameAs, sdLicense, inLanguage`. **Croissant-specific (optional)** — `citeAs, isLiveDataset, sdVersion`.
...
| 9 | Croissant top-level field mapping | **resolved**: verified against `docs/croissant-spec-1.1.md`@`401f6fff81db26a49c0d1704f02bffc4e4fa8fe2` — corrected `distribution` from "Croissant-specific" to required, see Q9/Step 4 |
